...
int main(int argc, char** argv)
{
    ...
    learnopengl::PointLight pointLight2;
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        pointLight1.setPosition(glm::vec3(std::cosf(timeAsRad) * 2.f + 2.f, 1.5f, 1.f) * 1.1f);
        pointLight2.setPosition(glm::vec3(std::cosf(timeAsRad) * 2.f + 2.f, 0.f, 1.f) * 1.1f);
...

        {
            ...
            {
                auto pointLight = row ? pointLight1 : pointLight2;
                pointLight.setPosition(view * glm::vec4(pointLight.position(), 1));
