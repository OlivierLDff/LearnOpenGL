...
int main(int argc, char** argv)
{
    ...
    camera.setCameraPos(glm::vec3(3.f, 1.f, 6.f));
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        const auto lightPos1 = glm::vec3(std::cosf(timeAsRad) * 4.f + 3.f, 2.f, 1.f) * 1.1f;
        const auto lightPos2 = glm::vec3(std::cosf(timeAsRad) * 4.f + 3.f, 0.f, 1.f) * 1.1f;
...

        {
            ...
            shaderProgram.setVec3("lightPos", lightPos1.x, lightPos1.y, lightPos1.z);
            for(int i = 0; i < 4; ++i)
            {
                glm::mat4 model = glm::mat4(1.0f);
                //model = glm::rotate(model, glm::radians(-90.f), glm::vec3(0.f, 1.f, 0.f));
                model = glm::translate(model, glm::vec3(i * 2.f, 2.f, 0));
                //model = glm::scale(model, glm::vec3(1 / 8.f, 1 / 8.f, 1 / 8.f));
                ...
                shaderProgram.setFloat("specularStrength", 0.5f);
                shaderProgram.setFloat("shininess", std::max(1.f, std::powf(float(i), 6)));

                glBindVertexArray(VAO);
                glDrawArrays(GL_TRIANGLES, 0, 36);
            }

            shaderProgram.setVec3("lightPos", lightPos2.x, lightPos2.y, lightPos2.z);
            for(int i = 0; i < 4; ++i)
            {
                glm::mat4 model = glm::mat4(1.0f);
                //model = glm::rotate(model, glm::radians(-90.f), glm::vec3(0.f, 1.f, 0.f));
                model = glm::translate(model, glm::vec3(float(i) * 2.f, 0, 0));
                //model = glm::scale(model, glm::vec3(1 / 8.f, 1 / 8.f, 1 / 8.f));
                shaderProgram.setMat4("model", glm::value_ptr(model));
                ...
                shaderProgram.setFloat("ambientStrength", 0.1f);
                shaderProgram.setFloat("specularStrength", 4.f / float(4.f * i + 1));
                shaderProgram.setFloat("shininess", 64.f);
