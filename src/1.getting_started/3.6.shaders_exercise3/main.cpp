...
int main(int argc, char** argv)
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        const float horizontalOffset = float(std::sin(timeValue) / 2);
        const float verticalOffset = float(std::cos(timeValue) / 2);
        shaderProgram.setFloat("horizontalOffset", horizontalOffset);
        shaderProgram.setFloat("verticalOffset", verticalOffset);
