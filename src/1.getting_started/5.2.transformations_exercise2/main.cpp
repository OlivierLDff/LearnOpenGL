...
int main(int argc, char** argv)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
...

        {
            ...
            trans = glm::scale(trans, glm::vec3(std::cos((float)glfwGetTime()), std::sin((float)glfwGetTime()) * 0.5f, 0));
        ...
        {
            glm::mat4 trans = glm::mat4(1.0f);
            trans = glm::scale(trans, glm::vec3(std::fabs(std::sin((float)glfwGetTime())), std::fabs(std::cos((float)glfwGetTime()) * 0.5f), 0));
            trans = glm::rotate(trans, (float)glfwGetTime() * -0.5f, glm::vec3(0.0, 0.0, 1.0));
            trans = glm::translate(trans, glm::vec3(0.5f, 0.5f, 0.0f));
